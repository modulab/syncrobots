 ...
 */
class Agent {
    final List<Pair<Double, Agent>> agentNeighbors = new ArrayList<Pair<Double, Agent>>();
    final List<Pair<Double, Obstacle>> obstacleNeighbors = new ArrayList<Pair<Double, Obstacle>>();
        ...
        if (this != agent) {
            ...
            if (distSq < rangeSq) {
                if (agentNeighbors.size() < maxNeighbors) {
                    agentNeighbors.add(new Pair<Double, Agent>(distSq, agent));
                }
                ...
                agentNeighbors.set(i, new Pair<Double, Agent>(distSq, agent));
    ...
    void insertObstacleNeighbor(Obstacle obstacle, double rangeSq) {
        ...
        if (distSq < rangeSq) {
            obstacleNeighbors.add(new Pair<Double, Obstacle>(distSq, obstacle));
            ...
            obstacleNeighbors.set(i, new Pair<Double, Obstacle>(distSq, obstacle));
        }
    }
    ...
    private void linearProgram3(int numObstacleLines, int beginLine) {
        ...
        for (int i = beginLine; i < lines.size(); i++) {
            if (RVOMath.det(lines.get(i).direction, lines.get(i).point.subtract(newVelocity)) > distance) {
                ...
                final List<Line> projectedLines = new ArrayList<Line>(numObstacleLines);
