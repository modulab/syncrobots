    ...
    private ObstacleTreeNode buildObstacleTreeRecursive(List<Obstacle> obstacles) {
        ...
        for (int i = 0; i < obstacles.size(); i++) {
            ...
            for (int j = 0; j < obstacles.size(); j++) {
                ...
                final Pair<Integer, Integer> pair1 = new Pair<Integer, Integer>(FastMath.max(leftSize, rightSize), FastMath.min(leftSize, rightSize));
                final Pair<Integer, Integer> pair2 = new Pair<Integer, Integer>(FastMath.max(minLeft, minRight), FastMath.min(minLeft, minRight));
        ...
        // Build split node.
        final List<Obstacle> leftObstacles = new ArrayList<Obstacle>(minLeft);

        for (int n = 0; n < minLeft; n++) {
            leftObstacles.add(null);
        }

        final List<Obstacle> rightObstacles = new ArrayList<Obstacle>(minRight);
